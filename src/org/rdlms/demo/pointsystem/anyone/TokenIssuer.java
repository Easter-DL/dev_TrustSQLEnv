...
package org.rdlms.demo.pointsystem.anyone;
...
public class TokenIssuer {
	...
	public static int createIssuerTable(String issuer_name, String service_name, String token_name, String isser_wallet_name, String orderer_wallet_name) {
		...
		/* TABLE SCHEMA OPTIONS for TRUST
			{HA_OPTION_TYPE_STRING,"DSA_SCHEME",sizeof("DSA_SCHEME"),
			{HA_OPTION_TYPE_STRING,"TABLE_ISSUER_PUB_KEY",sizeof("TABLE_ISSUER_PUB_KEY"),
			{HA_OPTION_TYPE_STRING,"TRUSTED_REFERENCE_PUB_KEY",sizeof("TRUSTED_REFERENCE_PUB_KEY"),
			{HA_OPTION_TYPE_STRING,"TOSA_PUB_KEY",sizeof("TOSA_PUB_KEY"),
			{HA_OPTION_TYPE_STRING,"TOSA_MASTER_PUB_KEY",sizeof("TOSA_MASTER_PUB_KEY"),
			{HA_OPTION_TYPE_STRING,"TOSA_MASTER_PUB_KEY",sizeof("TOSA_MASTER_PUB_KEY"),
			{HA_OPTION_TYPE_STRING,"TABLE_SCHEMA_SIGN",sizeof("TABLE_SCHEMA_SIGN"),
			{HA_OPTION_TYPE_STRING,"TABLE_TRUSTEDCHAIN_SIGN",sizeof(" TABLE_TRUSTEDCHAIN_SIGN"),
			{HA_OPTION_TYPE_STRING,"TRIGGER_BEFORE_INSERT_SIGN",sizeof("TRIGGER_BEFORE_INSERT_SIGN"),
			{HA_OPTION_TYPE_STRING,"TRIGGER_AFTER_INSERT_SIGN",sizeof("TRIGGER_AFTER_INSERT_SIGN")
		*/
		...
		strTrgStmt += "DECLARE _senders_receive_total BIGINT DEFAULT 0; "+newline;
		strTrgStmt += "DECLARE _senders_send_total BIGINT DEFAULT 0; "+newline;
		strTrgStmt += newline;
		...
		strTrgStmt += newline;
		strTrgStmt += "	-- #3-1 SENDER's RECEIVE_TOTAL"+newline;
		strTrgStmt += "	SELECT IFNULL(SUM(AMOUNT),0) INTO _senders_receive_total"+newline;
		strTrgStmt += "	FROM TRANSACTIONS_"+token_name+newline;
		strTrgStmt += "	WHERE RECEIVER=NEW.SENDER;"+newline;
		strTrgStmt += newline;
		strTrgStmt += "	-- #3-1 SENDER's SEND_TOTAL "+newline;
		strTrgStmt += "	SELECT IFNULL(SUM(AMOUNT),0) INTO _senders_send_total "+newline;
		strTrgStmt += "	FROM TRANSACTIONS_"+token_name+newline;
		strTrgStmt += "	WHERE SENDER=NEW.SENDER; "+newline;
		strTrgStmt += newline;
		strTrgStmt += "	IF (NEW.AMOUNT > (_senders_receive_total-_senders_send_total)) THEN "+newline;
